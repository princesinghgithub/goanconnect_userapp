...
// const VEHICLE_TYPES = [
//   { id: "bike", label: "Bike", emoji: "🏍️", desc: "1 seat" },
//   { id: "auto", label: "Auto", emoji: "🛺", desc: "3 seats" },
//   { id: "car", label: "Cab", emoji: "🚗", desc: "4 seats" },
//   { id: "tractor", label: "Tractor", emoji: "🚜", desc: "Khet" },
//   { id: "jcb", label: "JCB", emoji: "🚧", desc: "Const." },
//   { id: "tempo", label: "Tempo", emoji: "🚐", desc: "Saman" },
// ];
...
//         dropAddress: drop.address,
//         vehicleType: selectedVehicle,
//         distance: String(calcDistance()),
//       },
//     });
//   };

//   // ✅ Pre-Book button — schedule-booking screen pe jaata hai
//   const handlePreBook = () => {
//     if (!pickup || !drop) {
//       Alert.alert("Location Daalo", "Pehle pickup aur drop select karo");
//       return;
//     }
//     router.push({
//       pathname: "/(booking)/schedule-booking",
//       params: {
//         pickupLat: String(pickup.latitude),
//         pickupLng: String(pickup.longitude),
//         pickupAddress: pickup.address,
...
//                   !drop && styles.locationPlaceholder,
//                 ]}
//                 numberOfLines={1}
//               >
//                 {drop?.address || "Kahan jaana hai? 🔍"}
...
//         {/* ✅ DO BUTTONS — Abhi Book + Pre-Book */}
//         <View style={styles.btnRow}>
//           {/* Abhi Ride Lo */}
//           <TouchableOpacity
//             style={[
//               styles.bookBtn,
//               (!pickup || !drop) && styles.bookBtnDisabled,
//             ]}
//             onPress={handleBookRide}
//             disabled={!pickup || !drop}
//           >
...
//             <Text style={styles.bookBtnText}>
//               {pickup && drop ? "Abhi Book Karo" : "Location chuniye"}
//             </Text>
//           </TouchableOpacity>
...
//             disabled={!pickup || !drop}
//           >
//             <Ionicons name="calendar" size={18} color="#1a3a5c" />
...
//   vehicleLabel: { fontSize: 11, fontWeight: "800", color: "#555" },
//   vehicleLabelActive: { color: "#F5A623" },
//   vehicleDesc: {
//     fontSize: 9,
//     color: "#aaa",
...
//   preBookBtnText: { color: "#1a3a5c", fontSize: 14, fontWeight: "800" },
// });
...
export default function HomeScreen() {
  ...
  // ✅ String refs — JSON compare ke liye, infinite loop rokne ke liye
  const pickupRef = useRef<string>("");
  ...
  // ✅ loadAll — sirf tab setState karo jab value badli ho
  const loadAll = async () => {
    const userData = await AsyncStorage.getItem("user");
    if (userData) {
      const name = JSON.parse(userData).name || "Dost";
      setUserName((prev) => (prev !== name ? name : prev));
    }

    const pickupRaw = await AsyncStorage.getItem("pickup");
    if (pickupRaw && pickupRaw !== pickupRef.current) {
      pickupRef.current = pickupRaw;
      setPickup(JSON.parse(pickupRaw));
    }

    const dropRaw = await AsyncStorage.getItem("drop");
    if (dropRaw && dropRaw !== dropRef.current) {
      dropRef.current = dropRaw;
      setDrop(JSON.parse(dropRaw));
    }
  };

  useEffect(() => {
    loadAll();
    const interval = setInterval(loadAll, 1000); // 500 → 1000ms
    return () => clearInterval(interval);
  }, []);
  ...
  const getLocation = async () => {
    try {
      ...
      const p = { ...coords, address: addressStr || "Current Location" };
      setPickup(p);
      pickupRef.current = JSON.stringify(p);
      await AsyncStorage.setItem("pickup", JSON.stringify(p));
      mapRef.current?.animateToRegion({
        ...coords,
        latitudeDelta: 0.02,
      ...
      <View style={styles.bottomSheet}>
        ...
        <View style={styles.btnRow}>
          ...
          <TouchableOpacity
            style={[
              styles.preBookBtn,
              (!pickup || !drop) && styles.preBookBtnDisabled,
            ]}
            onPress={handlePreBook}
            disabled={!pickup || !drop}
          >
