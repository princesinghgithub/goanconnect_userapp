...
const VEHICLE_OPTIONS = [
  { id: "bike", label: "Bike", emoji: "🏍️", seats: "1 Seat", perKm: 8 },
  { id: "auto", label: "Auto", emoji: "🛺", seats: "3 Seats", perKm: 12 },
  { id: "car", label: "Cab", emoji: "🚗", seats: "4 Seats", perKm: 15 },
];

function localFare(distance: number, perKm: number): number {
  const base = perKm * 2;
  return Math.round(base + distance * perKm);
}

function getETA(distance: number, vehicleId: string): string {
  const speed = vehicleId === "bike" ? 35 : vehicleId === "auto" ? 25 : 30;
  const minutes = Math.round((distance / speed) * 60);
  if (minutes < 60) return `${minutes} min`;
  return `${Math.floor(minutes / 60)}h ${minutes % 60}m`;
...
export default function FareEstimateScreen() {
  ...
  const fetchFareEstimate = async () => {
    ...
    try {
      ...
      // ✅ Sahi route: /ride/fare-estimate (rides nahi, ride hai!)
      const res = await axios.get(`${API_BASE_URL}/ride/fare-estimate`, {
        params: {
          pickupLat: params.pickupLat,
          pickupLng: params.pickupLng,
          dropLat: params.dropLat,
      ...
      if (res.data?.success && res.data?.data) {
        ...
        // Backend fares: { bike, auto, car, ... }
        // Frontend uses: bike, auto, car (cab → car map)
        const f = d.fares || {};
        setFareData({
          bike: f.bike || localFare(d.distance, 8),
          auto: f.auto || localFare(d.distance, 12),
          car: f.car || localFare(d.distance, 15),
        });
      } else {
        throw new Error("Bad response");
      }
    } catch (e) {
      console.log("Fare estimate fallback:", e);
      // ✅ Local fallback — toFixed crash nahi hoga
      const pLat = parseFloat(params.pickupLat || "0");
      const pLng = parseFloat(params.pickupLng || "0");
      const dLat = parseFloat(params.dropLat || "0");
      const dLng = parseFloat(params.dropLng || "0");
      const dist = haversine(pLat, pLng, dLat, dLng);
      setDistance(dist);
      setDuration(Math.round(dist * 2.5));
      setFareData({
        bike: localFare(dist, 8),
        auto: localFare(dist, 12),
        car: localFare(dist, 15),
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirm = () => {
    const fare = fareData[selectedVehicle] || 0;
    router.push({
      pathname: "/(booking)/confirm-booking",
      params: {
        pickupLat: params.pickupLat,
    ...
    <View style={styles.container}>
        ...
        {!loading && (
          <View style={styles.distanceBadge}>
            <Text style={styles.distanceText}>
              📏 {safeDistance.toFixed(1)} km • ⏱️ {duration} min
            </Text>
          </View>
        )}
      </View>
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
          ...
          {VEHICLE_OPTIONS.map((vehicle) => {
            const fare =
              fareData[vehicle.id] || localFare(safeDistance, vehicle.perKm);
              ...
              >
                <View style={styles.vehicleLeft}>
                  ...
                  <View>
                    ...
                    <Text style={styles.vehicleSeats}>{vehicle.seats}</Text>
                    <Text style={styles.vehicleEta}>
                      ⏱️ {getETA(safeDistance, vehicle.id)} • ₹{vehicle.perKm}
                      /km
                    </Text>
                  </View>
                </View>
          ...
          <View style={styles.paymentNote}>
            ...
            <Text style={styles.paymentNoteText}>
              Cash payment • Baad mein UPI bhi
            </Text>
          </View>
        </ScrollView>
      )}
      ...
      {!loading && (
        <View style={styles.bottomContainer}>
          <TouchableOpacity style={styles.confirmBtn} onPress={handleConfirm}>
            <Text style={styles.confirmBtnText}>
              {VEHICLE_OPTIONS.find((v) => v.id === selectedVehicle)?.emoji}{" "}
              Confirm Karo • ₹{fareData[selectedVehicle] || "—"}
            </Text>
          </TouchableOpacity>
        </View>
      )}
